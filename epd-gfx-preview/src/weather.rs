...
pub enum WMOCode {
    ...
    ThunderstormWitHeavyHail = 99,
}

pub fn fetch_current_weather(location: &Location) -> Result<OpenMeteoData> {
    ...
    let params = [
        ...
        ("daily", &"sunrise".to_owned()),
        ("timezone", &"Europe/Berlin".to_owned()),
    ];
    let res = client.get(base).query(&params).send()?;
    let body = res.bytes()?;
    println!("{body:?}");
    let data: OpenMeteoData = serde_json::from_slice(&body).context("Unable to decode response")?;
    println!("{data:?}");
