...
pub fn draw_pixel(fb: &mut [u8], x: usize, y: usize, color: u8) {
    if x as u32 >= WIDTH {
        return;
    }
    if y as u32 >= HEIGHT {
    ...
    let fb_index = y * WIDTH as usize / 2 + x / 2;
...
pub fn draw_hline(fb: &mut [u8], x: usize, y: usize, length: usize, color: u8) {
    for i in 0..length {
        let xx = x + i;
...
pub fn draw_vline(fb: &mut [u8], x: usize, y: usize, length: usize, color: u8) {
    for i in 0..length {
        let yy = y + i;
