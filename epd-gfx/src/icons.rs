...
use crate::{draw_line, draw_pixel, fill_circle, fill_rect};
...
pub fn addmoon(fb: &mut [u8], x: u32, y: u32, scale: u32, size: IconSize) {
    if size == IconSize::LARGE {
        fill_circle(fb, x - 85, y - 100, (scale as f32 * 0.8) as u32, BLACK);
        fill_circle(fb, x - 57, y - 100, (scale as f32 * 1.6) as u32, WHITE);
    } else {
        fill_circle(fb, x - 28, y - 37, (scale as f32 * 1.0) as u32, BLACK);
        fill_circle(fb, x - 20, y - 37, (scale as f32 * 1.6) as u32, WHITE);
    }
}

// Symbols are drawn on a relative 10x10grid and 1 scale unit = 1 drawing unit
pub fn addcloud(fb: &mut [u8], x: u32, y: u32, scale: u32, linesize: u32) {
    // Draw cloud outer
    fill_circle(fb, x - scale * 3, y, scale, BLACK); // Left most circle
    fill_circle(fb, x + scale * 3, y, scale, BLACK); // Right most circle
    fill_circle(fb, x - scale, y - scale, (scale as f32 * 1.4) as u32, BLACK); // left middle upper circle
    fill_circle(
        fb,
        x + (scale as f32 * 1.5) as u32,
        y - (scale as f32 * 1.3) as u32,
        (scale as f32 * 1.75) as u32,
        BLACK,
    ...
    fill_rect(
       ...
       // Clear cloud inner
    fill_circle(fb, x - scale * 3, y, scale - linesize, WHITE); // Clear left most circle
    fill_circle(fb, x + scale * 3, y, scale - linesize, WHITE); // Clear right most circle
    fill_circle(
        fb,
        x - scale,
        y - scale,
        (scale as f32 * 1.4) as u32 - linesize,
        WHITE,
    ...
    fill_circle(
        ...
        x + (scale as f32 * 1.5) as u32,
        y - (scale as f32 * 1.3) as u32,
        (scale as f32 * 1.75) as u32 - linesize,
        WHITE,
    ); // Right middle upper circle
    fill_rect(
        fb,
        x - scale * 3 + 2,
        y - scale + linesize - 1,
        (scale as f32 * 5.9) as u32,
        scale * 2 - linesize * 2 + 2,
        WHITE,
    ); // Upper and lower lines
}

pub fn addraindrop(fb: &mut [u8], mut x: u32, mut y: u32, scale: u32) {
    fill_circle(fb, x, y, scale / 2, BLACK);
    //fillTriangle(
    //    x - scale / 2,
    //    y,
    //    x,
    //    y - (scale as f32 * 1.2) as u32,
    //    x + scale / 2,
    //    y,
    //    BLACK,
    //);
    x = x + (scale as f32 * 1.6) as u32;
    y = y + scale / 3;
    fill_circle(fb, x, y, scale / 2, BLACK);
    //fillTriangle(
    //    x - scale / 2,
...
pub fn addrain(fb: &mut [u8], x: u32, y: u32, mut scale: u32, size: IconSize) {
    if (size == IconSize::SMALL) {
        scale = (scale as f32 * 1.34) as u32;
    }
    for d in 0..4 {
        addraindrop(
            fb,
            x + (scale as f32 * (7.8 - d as f32 * 1.95) - (scale as f32 * 5.2)) as u32,
            y + (scale as f32 * 2.1) as u32 - scale / 6,
            (scale as f32 / 1.6) as u32,
        );
    }
}

pub fn addsnow(fb: &mut [u8], x: u32, y: u32, scale: u32, size: IconSize) {
    let mut dxo;
    let mut dyo;
    let mut dxi;
    let mut dyi;
    for flakes in 0..5 {
        for i in (0..360).step_by(45) {
            dxo = 0.5 * scale as f32 * ((i as f32 - 90.0) * 3.14 / 180.0).cos();
            dxi = dxo * 0.1;
            dyo = 0.5 * scale as f32 * ((i as f32 - 90.0) * 3.14 / 180.0).sin();
            dyi = dyo * 0.1;
            draw_line(
                fb,
                (dxo + x as f32 + (flakes as f32 * 1.5 - 3.0) * scale as f32) as u32,
                (dyo + y as f32 + scale as f32 * 2.0) as u32,
                (dxi + x as f32 + (flakes as f32 * 1.5 - 3.0) * scale as f32) as u32,
                (dyi + y as f32 + scale as f32 * 2.0) as u32,
                BLACK,
            );
        }
    }
...
pub fn addtstorm(fb: &mut [u8], x: u32, mut y: u32, scale: u32) {
    y = y + scale / 2;
    //for i in 0..5 {
    //    draw_line(
    //        fb,
    //        x - scale * 4 + scale * i * 1.5 + 0,
    //        y + (scale as f32 * 1.5) as u32,
    //        x - (scale as f32 * 3.5) as u32 + scale * i * 1.5 + 0,
    //        y + scale,
    //        BLACK,
    //    );
    //    if (scale != Small) {
    ...
    //            y + (scale as f32 * 1.5) as u32,
    //            x - (scale as f32 * 3.5) as u32 + scale * i * 1.5 + 1,
    //            y + scale,
    //            BLACK,
    //        );
    //        draw_line(
    ...
    //            y + (scale as f32 * 1.5) as u32,
    //            x - (scale as f32 * 3.5) as u32 + scale * i * 1.5 + 2,
    //            y + scale,
    //            BLACK,
    //        );
    //    }
...
pub fn addsun(fb: &mut [u8], x: u32, y: u32, scale: u32, size: IconSize) {
    let mut linesize = 3;
    if (size == IconSize::SMALL) {
        linesize = 1;
    }
    fill_rect(fb, x - scale * 2, y, scale * 4, linesize, BLACK);
    fill_rect(fb, x, y - scale * 2, linesize, scale * 4, BLACK);
    draw_line(
        fb,
        x - (scale as f32 * 1.3) as u32,
        y - (scale as f32 * 1.3) as u32,
        x + (scale as f32 * 1.3) as u32,
        y + (scale as f32 * 1.3) as u32,
        BLACK,
    ...
    fill_circle(fb, x, y, (scale as f32 * 1.3) as u32, WHITE);
    fill_circle(fb, x, y, scale, BLACK);
    fill_circle(fb, x, y, scale - linesize, WHITE);
}

pub fn addfog(fb: &mut [u8], x: u32, mut y: u32, scale: u32, mut linesize: u32, size: IconSize) {
    if (size == IconSize::SMALL) {
        y -= 10;
        linesize = 1;
    }
...
pub fn Sunny(fb: &mut [u8], x: u32, mut y: u32, size: IconSize, name: &str) {
    let mut scale: u32 = Small;
    if size == IconSize::LARGE {
        scale = Large;
    } else {
    ...
    scale = (scale as f32 * 1.6) as u32;
    addsun(fb, x, y, scale, size);
}

pub fn MostlySunny(fb: &mut [u8], x: u32, y: u32, size: IconSize, name: &str) {
    let mut scale: u32 = Small;
    let mut linesize = 3;
    let mut offset = 5;
    if (size == IconSize::LARGE) {
    ...
    addsun(
        fb,
        x - (scale as f32 * 1.8) as u32,
        y - (scale as f32 * 1.8) as u32 + offset,
        scale,
        size,
    );
}

pub fn MostlyCloudy(fb: &mut [u8], x: u32, y: u32, size: IconSize, name: &str) {
    let mut scale: u32 = Small;
    let mut linesize = 3;
    if (size == IconSize::LARGE) {
        scale = Large;
...
pub fn Cloudy(fb: &mut [u8], x: u32, mut y: u32, size: IconSize, name: &str) {
    let mut scale: u32 = Large;
    let mut linesize = 3;
    if (size == IconSize::SMALL) {
        scale = Small;
        if (name.ends_with("n")) {
        ...
        addcloud(fb, x + 30, y - 45, 5, linesize); // Cloud top right
        addcloud(fb, x - 20, y - 30, 7, linesize); // Cloud top left
        addcloud(fb, x, y, scale, linesize); // Main cloud
    }
}

pub fn Rain(fb: &mut [u8], x: u32, y: u32, size: IconSize, name: &str) {
    let mut scale: u32 = Large;
    let mut linesize = 3;
    if (size == IconSize::SMALL) {
        scale = Small;
...
pub fn ExpectRain(fb: &mut [u8], x: u32, y: u32, size: IconSize, name: &str) {
    let mut scale: u32 = Large;
    let mut linesize = 3;
    if (size == IconSize::SMALL) {
        scale = Small;
...
pub fn ChanceRain(fb: &mut [u8], x: u32, y: u32, size: IconSize, name: &str) {
    let mut scale: u32 = Large;
    let mut linesize = 3;
    if (size == IconSize::SMALL) {
        scale = Small;
...
pub fn Tstorms(fb: &mut [u8], x: u32, y: u32, size: IconSize, name: &str) {
    let mut scale: u32 = Large;
    let mut linesize = 3;
    if (size == IconSize::SMALL) {
        scale = Small;
...
pub fn Snow(fb: &mut [u8], x: u32, y: u32, size: IconSize, name: &str) {
    let mut scale: u32 = Large;
    let mut linesize = 3;
    if (size == IconSize::SMALL) {
        scale = Small;
...
pub fn Fog(fb: &mut [u8], x: u32, y: u32, size: IconSize, name: &str) {
    let mut linesize = 3;
    let mut scale = Large;
    if (size == IconSize::SMALL) {
        scale = Small;
...
pub fn Haze(fb: &mut [u8], x: u32, y: u32, size: IconSize, name: &str) {
    let mut linesize = 3;
    let mut scale = Large;
    if (size == IconSize::SMALL) {
        scale = Small;
    ...
    addsun(fb, x, y - 5, (scale as f32 * 1.4) as u32, size);
    addfog(fb, x, y - 5, (scale as f32 * 1.4) as u32, linesize, size);
}

pub fn CloudCover(fb: &mut [u8], x: u32, y: u32, CCover: u32) {
    addcloud(fb, x - 9, y - 3, (Small as f32 * 0.8) as u32, 2); // Cloud top left
    addcloud(fb, x + 3, y - 3, (Small as f32 * 0.8) as u32, 2); // Cloud top right
    addcloud(fb, x, y, (Small as f32 * 0.8) as u32, 2); // Main cloud
                                                        //drawString(x + 44, y - 14, String(CCover) + "%", EPD_DRAW_ALIGN_LEFT);
}

pub fn Visibility(fb: &mut [u8], x: u32, mut y: u32, visibility: &str) {
    y = y - 3;
